      ...
      return (
        <Drawer.Portal>
          ...
          <Drawer.Content
            ref={ref}
            className={cn(
              'border-border bg-card fixed inset-x-0 bottom-0 z-50 flex max-h-[90dvh] flex-col rounded-t-xl border-t outline-none',
              className,
            )}
            style={style}
            {...props}
          >
            <Drawer.Handle className="bg-muted-foreground/40 mx-auto my-4 h-1 w-9 shrink-0 rounded-full" />
            {managed ? (
              <>{children}</>
            ) : (
      ...
      <DialogPrimitive.Portal>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <DialogPrimitive.Content
            ref={ref}
            className={cn(
              'border-border bg-card relative max-h-[90dvh] w-full max-w-lg rounded-xl border shadow-2xl',
